...
fun MyCardsScreen(
    ...
    // Observar eventos do ViewModel de gerenciamento
    LaunchedEffect(Unit) {
        cardManagementViewModel.events.collect { event ->
            when (event) {
                is CardManagementEvent.CardDeletedSuccessfully -> {
                    viewModel.load()
                    // Se não houver mais cartões após exclusão, ir para criação
                    val updatedCards = state.data ?: emptyList()
    ...
    // Wrapper Box para colocar o modal por cima de toda a tela
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        // Modal flutuante de gerenciamento - por cima do Scaffold
        ...
        if (selectedCard != null) {
            CardManagementBottomSheet(
                ...
                visible = managementUiState.isManagementSheetOpen
            )
        }

        // Dialogs de exclusão - por cima de tudo
        if (managementUiState.showDeleteWarningDialog && selectedCard != null) {
